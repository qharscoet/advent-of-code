...
fn euclid(a: u64, b: u64) -> (u64, i64, i64) {
    fn euclid_in(ruv_i: (u64, i64, i64), ruv_i2: (u64, i64, i64)) -> (u64, i64, i64) {
        if ruv_i.0 == 0 {
            ...
            let new_ruv = (
                ruv_i2.0 - q * ruv_i.0,
                ruv_i2.1 - (q as i64) * ruv_i.1,
                ruv_i2.2 - (q as i64) * ruv_i.2,
            );
...
fn main() {
    ...
    let buses: Vec<(u32, u32)> = lines.next().unwrap().split(',')
        .enumerate()
    ...
    let equations: Vec<(u32, u32)> = buses
        .iter()
        .map(|(idx, id)| {
            let (idx_i32, id_i32) = (*idx as i32, *id as i32);
            (
                ((-idx_i32 % id_i32 + id_i32) as u32) % id, // positive modulus
                *id,
            )
        })
        .collect();
