...
struct Universe {
    grid: HashSet<Coord>,
    min_dim: (i32, i32, i32, i32),
    max_dim: (i32, i32, i32, i32),
}
...
impl Universe {
        ...
        for i in self.min_dim.0 - 1..=self.max_dim.0 + 1 {
            for j in self.min_dim.1 - 1..=self.max_dim.1 + 1 {
                for k in self.min_dim.2 - 1..=self.max_dim.2 + 1 {
                    for l in self.min_dim.3 - 1..=self.max_dim.3 + 1 {
                        let active_neigbours = self.count_active_neighbours((i, j, k, l));
                        if (self.grid.contains(&(i, j, k, l)) && (active_neigbours == 2 || active_neigbours == 3))
                            || (!self.grid.contains(&(i, j, k, l)) && active_neigbours == 3)
                        {
                            new_grid.insert((i, j, k, l));
                        }
                    }
                }
            }
    ...
    /* Disabled for part 2 because flemme */
    // fn display(&self) {
        ...
        buf_reader
            ...
            .fold(HashSet::new(), |acc, (idx_l, line)| {
                let line_set = line
                    ...
                    .filter_map(|(idx_c, c)| {
                        if c == '#' {
                            Some((idx_l as i32, idx_c as i32, 0i32, 0i32))
                        } else {
                            None
                        }
                    })
